...
from chem_eng_solver.base_solver import EqRegistry, Solver, UNKNOWN
...
class Fluids(Solver):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.initial = {
            k: self._convert_units(k, v) for k, v in initial.items()
        }
        self.final = {k: self._convert_units(k, v) for k, v in final.items()}

    def bernoulli(self) -> str:
        """
        Finds roots to Bernoulli equation on the principle of conservation of
        energy (initial = final --> initial - final = 0). Uses
        `scipy.optimize.brentq` as algorithm for finding all roots. Note that
        multiple solutions to the equation may exist depending on the term being
        solved for (e.g. v is squared, so both + and - roots will exist when
        solving for v_initial or v_final).

        Method saves solution to :attr:`solutions` in SI units as well as
        returning the string representation of solved roots.
        ...
        self.solutions = [self._add_units(root, FluidsEq) for root in solutions]
        return (
            f"{self.unknown} = "
